...
@php
    ...
    $script = '<script>
        $(".remove-item-btn").on("click", function() {
            ...
            console.log(id);
            Swal.fire({
                title: "<h6>¿Estás seguro de eliminar este proveedor?</h6>",
...
@section('content')
    <div class="card h-100 p-0 radius-12">
        <div
            ...
            <form method="GET" action="{{ route('proveedor') }}" class="d-flex align-items-center flex-wrap gap-3">
                ...
                <a href="{{ route('proveedor') }}" class="btn btn-sm btn-outline-secondary">Reset</a>
            </form>
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    <div class="d-flex align-items-center gap-10 justify-content-center">
                                        <a href="{{ route('proveedor.editar', $prov->cod_proveedor) }}"
                                        ...
                                        <button type="button"
                                            ...
                                            data-id="{{ $prov->cod_proveedor }}">
                                            <iconify-icon icon="fluent:delete-24-regular" class="menu-icon"></iconify-icon>
                                        </button>
                                        <form action="{{ route('proveedor.eliminar', $prov->cod_proveedor) }}"
                                            method="POST" id="form-eliminar-{{ $prov->cod_proveedor }}">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                    </div>
                ...
                <ul class="pagination d-flex flex-wrap align-items-center gap-2 justify-content-center">
                    ...
                    <li class="page-item {{ $proveedores->onFirstPage() ? 'disabled' : '' }}">
                        <a class="page-link bg-neutral-300 text-secondary-light fw-semibold radius-8 border-0 d-flex align-items-center justify-content-center h-32-px w-32-px text-md"
                            href="{{ $proveedores->previousPageUrl() ?? 'javascript:void(0)' }}">
                    ...
                    @foreach ($proveedores->links()->elements[0] as $page => $url)
                        <li class="page-item {{ $proveedores->currentPage() == $page ? 'active' : '' }}">
                            <a class="page-link fw-semibold radius-8 border-0 d-flex align-items-center justify-content-center h-32-px w-32-px text-md {{ $proveedores->currentPage() == $page ? 'bg-primary-600 text-white' : 'bg-neutral-300 text-secondary-light' }}"
                                href="{{ $url }}">
                                {{ $page }}
                            </a>
                        </li>
                    ...
                    <li class="page-item {{ $proveedores->hasMorePages() ? '' : 'disabled' }}">
                        <a class="page-link bg-neutral-300 text-secondary-light fw-semibold radius-8 border-0 d-flex align-items-center justify-content-center h-32-px w-32-px text-md"
                            href="{{ $proveedores->nextPageUrl() ?? 'javascript:void(0)' }}">
